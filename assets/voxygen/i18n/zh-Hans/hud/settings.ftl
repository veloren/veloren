...
hud-settings-show_hitboxes = 显示碰撞体
...
hud-settings-damage_accumulation_duration = 伤害累积持续时间
hud-settings-incoming_damage = 所受伤害
hud-settings-incoming_damage_accumulation_duration = 受到的伤害累积持续时间
hud-settings-round_damage = 伤害数值取整
...
hud-settings-energybar_numbers = 耐力条数值
hud-settings-always_show_bars = 始终显示耐力条
hud-settings-experience_numbers = 经验值
hud-settings-accumulate_experience = 显示经验累计值
...
hud-settings-gpu_profiler = 启用GPU计时功能（并非所有设备都支持）
hud-settings-particles = 粒子
hud-settings-lossy_terrain_compression = 有损地形压缩
hud-settings-weapon_trails = 武器轨迹
hud-settings-flashing_lights = 闪烁光源
hud-settings-flashing_lights_info = 启用各类闪烁效果，例如屏幕闪烁或闪电击中特效
...
hud-settings-rain_occlusion-resolution = 雨水遮挡分辨率
hud-settings-lod_detail = LoD细节
hud-settings-reset_graphics = 重置为默认
hud-settings-bloom = 全局光晕
hud-settings-point_glow = 点光源发光
...
hud-settings-english_fallback = 为缺失的翻译显示英文
...
hud-settings-third_party_integrations = 第三方集成
hud-settings-enable_discord_integration = 启用 Discord 集成
hud-settings-autopress_behavior-toggle = 切换
hud-settings-autopress_behavior-auto = 自动
hud-settings-prefix_switch_point = 切换到国际单位制前缀的数位限制
hud-settings-walking_speed_behavior = 行走速度模式
hud-settings-walking_speed = 行走速度
hud-settings-zoom_lock_behavior = 相机缩放模式锁定
hud-settings-zoom_lock = 视角缩放锁定
hud-settings-draggable_windows = 可拖拽窗口
hud-settings-slots = 槽位
hud-settings-use_prefixes = 使用国际单位制
hud-settings-use_health_prefixes = 对生命值数值使用国际单位制词头，例如 1,000 显示为 1k
hud-settings-enable_poise_bar = 启用韧性条
hud-settings-minimap = 小地图
hud-settings-minimap_scale = 缩放比例
hud-settings-colored_player_marker = 彩色玩家标记
hud-settings-lock_chat = 锁定聊天窗口大小/位置
hud-settings-rain = 下雨效果
hud-settings-rain_ambience = 雨天环境音
hud-settings-sfx_channels = 音效通道容量
hud-settings-sfx_channels_low = 低
hud-settings-sfx_channels_med = 中
hud-settings-sfx_channels_high = 高
hud-settings-language_send_to_server = 将你设置的语言发送给服务器，(对服务器规则和每日消息进行本地化处理)
hud-settings-gamepad = 手柄
hud-settings-keyboard = 键盘
hud-settings-game_buttons = 游戏按键
hud-settings-game_layers = 游戏层级
hud-settings-menu_buttons = 菜单按键
hud-settings-row_background_opacity = 行背景透明度
hud-settings-aim_offset_x = 水平瞄准补偿
hud-settings-aim_offset_y = 竖直瞄准补偿
hud-settings-entities_detail_distance = 实体细节距离
hud-settings-present_mode-vsync_adaptive = 自适应垂直同步
...
hud-settings-ultra_graphics = 极高
hud-settings-combat_music = 战斗音乐
