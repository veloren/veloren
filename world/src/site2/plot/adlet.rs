...
pub struct AdletStronghold {
    ...
    structure_locations: Vec<(AdletStructure, Vec3<i32>, Dir)>,
    tunnels: Tunnels,
}
...
impl AdletStronghold {
    pub fn generate(wpos: Vec2<i32>, land: &Land, rng: &mut impl Rng) -> Self {
        ...
        let mut structure_locations = Vec::<(AdletStructure, Vec3<i32>, Dir)>::new();
        for _ in 0..desired_structures {
            if let Some((hut_loc, kind)) = attempt(50, || {
                ...
                let structure_kind = match rng.gen_range(0..10) {
                    0 => AdletStructure::Igloo,
                    1..=3 => AdletStructure::VeloriteHut,
                    4..=5 => AdletStructure::Banner,
                    _ => AdletStructure::Hut,
                };
        ...
        for (point, next_point) in self.wall_segments.iter() {
            ...
            (0..(SECTIONS_PER_WALL_SEGMENT as i32))
                ...
                .for_each(|(point, next_point)| {
                    ...
                    let lightstone = Fill::Brick(BlockKind::Wood, Rgb::new(107, 107, 107), 18);
                    let midstone = Fill::Brick(BlockKind::Wood, Rgb::new(70, 70, 70), 18);
                    let darkstone = Fill::Brick(BlockKind::Wood, Rgb::new(42, 42, 42), 18);
                    let darkpelt = Fill::Brick(BlockKind::Wood, Rgb::new(80, 47, 13), 35);
                    let lightpelt = Fill::Brick(BlockKind::Wood, Rgb::new(54, 25, 1), 25);
                    ...
                    let highwall =
                        painter.segment_prism(start, end, wall_mid_thickness, wall_mid_height);
                    painter.fill(highwall, lightstone.clone());

                    painter
                        .segment_prism(start, end, wall_mid_thickness + 1.0, wall_mid_height - 8.0)
                        .fill(midstone.clone());
                    let wallexterior = painter
                        .segment_prism(start, end, wall_mid_thickness + 2.0, wall_mid_height - 15.0)
                        .translate(Vec3::new(0, 0, 8));
                    let wallstrut = painter
                        .segment_prism(start, end, wall_mid_thickness - 1.0, wall_mid_height)
                        .translate(Vec3::new(0, 0, 8));
                    let wallexteriorlow = wallexterior.translate(Vec3::new(0, 0, -2));
                    ...
                    let cyl = painter.cylinder_with_radius(
                        Vec3::new(start.x as i32, start.y as i32, start.z as i32),
                        5.0,
                        wall_mid_height - 8.0,
                    );
                    ...
                    let startshift =
                        Vec3::new(randstart.x * 3.0, randstart.y * 3.0, randstart.z * 1.0);
                    let endshift = Vec3::new(randend.x * 3.0, randend.y * 3.0, randend.z * 1.0);

                    painter
                        .cubic_bezier(start, mid + startshift, mid + endshift, end, 2.5)
                        .translate(Vec3::new(0, 0, 27))
                        .intersect(overtop)
        ...
        self.structure_locations
            ...
            .for_each(|(kind, loc, door_dir)| {
                ...
                match kind {
                    ...
                    AdletStructure::Igloo => {
                        let igloo_pos = wpos.with_z(alt);

                        lazy_static! {
                            pub static ref IGLOO: AssetHandle<StructuresGroup> =
                                PrefabStructure::load_group("site_structures.adlet.igloo");
                        }

                        let igloo = IGLOO.read();
                        let igloo = igloo[self.seed as usize % igloo.len()].clone();

                        painter
                            .prim(Primitive::Prefab(Box::new(igloo.clone())))
                            .translate(igloo_pos)
                            .fill(Fill::Prefab(Box::new(igloo), igloo_pos, self.seed));
                    },
                    AdletStructure::ChieftainHut => {
                        let roof_height = 3.0;
