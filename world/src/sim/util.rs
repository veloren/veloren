...
impl PartialOrd for RiverKind {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        match (*self, *other) {
            ...
            (_, RiverKind::River { .. }) => Some(Ordering::Greater),
        }
    }
}
...
pub fn get_rivers(
    ...
    for &chunk_idx in newh.into_iter().rev() {
        ...
        let lake_idx = if indirection_idx < 0 {
            ...
            let neighbor_pass_idx = downhill_idx/* as usize*/;
            ...
            let mut lake_direction =
                neighbor_coef * (neighbor_pass_pos - pass_pos).map(|e| e as f64);
        ...
        if slope == 0.0 {
            ...
            // This should only ever happen when a lake flows into another lake, or to an
        ...
        if is_river {
            ...
            Vec2::new(dx, dy)*/
            let max_width = TerrainChunkSize::RECT_SIZE.x as f64 * CONFIG.river_max_width as f64; //neighbor_distance;
                                                                                                  //
        ...
        // 0.0 // Egress with no outgoing flows.
            ...
            true {
                ...
                mag_slope > max_slope {
                    ...
                    // sums += slope;
                    sums += max_slope;
                    new_h_i = slope * neighbor_distance / CONFIG.mountain_scale as f64 + h_j/* - old_h_i*/;
                } else {
                    sums += mag_slope;
                    // Just use the computed rate.
                }
