use common::{
    ...
    comp::{
        Alignment, Arc, Body, Buffs, CharacterState, Combo, Energy, Group, Health, Inventory, Mass,
        Ori, Player, Pos, Scale, Stats, aura::EnteredAuras,
    },
    event::{
...
use specs::{Entities, Join, Read, ReadStorage, SystemData, WriteStorage, shred};
...
impl<'a> System<'a> for Sys {
    ...
    fn run(_job: &mut Job<Self>, (read_data, mut arcs, mut positions, outcomes): Self::SystemData) {
        ...
        for (entity, arc) in (&read_data.entities, &mut arcs).join() {
            ...
            let arc_pos = if let Some(tgt_pos) = last_target.and_then(|e| positions.get(e)).copied()
            {
                ...
                .join()
            {
                ...
                if hit {
                    ...
                    // Once we hit the first entity, break the loop since we only arc to a single
                    // entity
