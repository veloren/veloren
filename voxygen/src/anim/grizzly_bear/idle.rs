...
impl Animation for IdleAnimation {
    ...
    fn update_skeleton(
        ...
        next.grizzly_bear_upper_head.offset =
            Vec3::new(0.0, 12.0, 16.0 + wave_ultra_slow * 1.0) / 8.46;
        next.grizzly_bear_upper_head.ori =
            Quaternion::rotation_y(wave_slow_cos * 0.015 + grizzly_bear_look.y)
                * Quaternion::rotation_x(grizzly_bear_look.x);
        next.grizzly_bear_upper_head.scale = Vec3::one() / 8.46;

        next.grizzly_bear_lower_head.offset = Vec3::new(0.0, 9.0, 4.5 + wave_ultra_slow * 0.1);
        next.grizzly_bear_lower_head.ori = Quaternion::rotation_y(wave_slow_cos * 0.015);
        next.grizzly_bear_lower_head.scale = Vec3::one();

        next.grizzly_bear_upper_torso.offset =
            Vec3::new(0.0, 5.5, 13.0 + wave_ultra_slow * 1.2) / 8.46;
