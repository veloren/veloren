...
impl MapFigureMgr {
    ...
    pub fn maintain(&mut self, renderer: &mut Renderer, client: &Client) {
        ...
        for (entity, _player, pos, vel, ori, scale, body, character, last_character, stats) in (
            &ecs.entities(),
            ...
            .join()
        {
            ...
            // TODO (haslersn): scale must depend on map LoD.
            let scale = 24.0 * scale.map(|s| s.0).unwrap_or(1.0);
