        ...
        *rate = 1.0;
        ...
        match active_tool_kind {
            ...
            Some(ToolKind::Hammer) => match ability_id {
                Some("common.abilities.custom.cyclops.hammer_shockwave") => {
                    next.second.scale = Vec3::one() * 0.0;
                ...
                Some("common.abilities.custom.dwarves.forgemaster.lavawave") => {
                    next.head.orientation = Quaternion::rotation_x(move1pow * 0.8 + move2 * -1.2);
                    next.control_l.position = Vec3::new(-0.5, 8.0, 1.0);
                    next.control_r.position = Vec3::new(-0.5, 8.0, 1.0);
                    next.control_l.orientation = Quaternion::rotation_x(PI / 2.0);
                    next.control_r.orientation = Quaternion::rotation_x(PI / 2.0);
                    next.weapon_l.position =
                        Vec3::new(-12.0 + (move1pow * 20.0).min(10.0), 3.0, -25.0);
                    next.weapon_r.position =
                        Vec3::new(12.0 + (move1pow * -20.0).max(-10.0), 3.0, -25.0);
