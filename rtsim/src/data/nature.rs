...
impl Nature {
    ...
    #[inline]
    pub fn chunk_resources(&self, key: Vec2<i32>) -> Option<&EnumMap<TerrainResource, f32>> {
        self.chunks.get(key).map(|c| &c.res)
    }

    #[inline]
    pub fn chunk_resources_mut(
        &mut self,
        key: Vec2<i32>,
    ) -> Option<&mut EnumMap<TerrainResource, f32>> {
        self.chunks.get_mut(key).map(|c| &mut c.res)
    }
}
...
pub struct Chunk {
    ...
    pub res: EnumMap<TerrainResource, f32>,
}
